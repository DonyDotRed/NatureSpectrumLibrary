...
- Decay Lab: multi-nuclide half-life decay chart with Origin + half-life quick filters, bulk filtered selection, default X = Time (d), Y = Activity (mCi), unit conversion, independent X/Y linear-log scales, axis swap
- 46 references
...
## v1.3 Decay Lab
- Multi-nuclide physical decay line chart
- Origin quick filters using the original dataset categories, including fission products and CRUD
- Half-life quick filters: short (<1 d), medium (1 d–<1 y), long (≥1 y)
- Select/Clear all nuclides matching the active filters while keeping individual checkbox selection
- Default X = Time (d), Y = Activity (mCi)
- X/Y axis swap and independent linear/log scales
- Activity/time unit conversion, normalized %, readout and CSV export
